...
# Initialize Matplotlib plot for runtime
fig, ax = plt.subplots()
runtimes = []
frame_count = 0

while True:
    start_time = time.time()  # Start time for each frame
    data_aux = []
    x_ = []
    y_ = []
    ret, frame = cap.read()
    ...
    # Update the Matplotlib plot with the current runtime
    ax.clear()
    ax.plot(runtimes, label='Runtime')
...
# Close the Matplotlib plot when the program ends
plt.close()
cap.release()
